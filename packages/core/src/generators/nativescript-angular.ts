...
export interface ToNativescriptAngularOptions extends BaseTranspilerOptions {
  standalone?: boolean;
...
interface NativescriptAngularBlockOptions {
  childComponents?: string[];
}

const mappers: {
  [key: string]: (json: MitosisNode, options: ToNativescriptAngularOptions) => string;
} = {
  Fragment: (json, options) => {
    return `<ng-container>${json.children
      .map((item) => blockToNativescriptAngular(item, options))
      .join('\n')}</ng-container>`;
  },
  Slot: (json, options) => {
    const renderChildren = () =>
      json.children?.map((item) => blockToNativescriptAngular(item, options)).join('\n');
  ...
  Image: (json, options) => {
    return (
      `<Image src="${json.properties.src}"></Image>`
    );
  },
  input: (json, options) => {
    return `<TextField class="${json.properties.class}" ${json.bindings?.value?.code ? '[text]="' + json.bindings.value.code + '"' : ''}></TextField>`;
  },
  textarea: (json, options) => {
    return `<TextView class="${json.properties.class}" ${json.bindings?.value?.code ? '[text]="' + json.bindings.value.code + '"' : ''}></TextView>`;
  },
  button: (json, options) => {
    return `<Button></Button>`;
  },
  select: (json, options) => {
    return `<ListPicker></ListPicker>`;
  },
  option: (json, options) => {
    return `<ListPickerItem></ListPickerItem>`;
  },
  form: (json, options) => {
...
const generateNativescriptNgModule = (
  content: string,
  name: string,
...
export const blockToNativescriptAngular = (
  json: MitosisNode,
  options: ToNativescriptAngularOptions = {},
  blockOptions: NativescriptAngularBlockOptions = {},
): string => {
  ...
  if (json.properties._text) {
    ...
    return `<Label text="${json.properties._text.trim()}"></Label>`;
  }
  ...
  if (checkIsForNode(json)) {
    ...
    str += json.children.map((item) => blockToNativescriptAngular(item, options, blockOptions)).join('\n');
    str += `</ng-container>`;
    ...
    let elSelector = childComponents.find((impName) => impName === json.name)
      ? kebabCase(json.name)
      : json.name;

    if (elSelector === 'div') {
      if (style?.display === 'flex') {
        elSelector = `FlexboxLayout flexDirection=${
          style?.display === 'flex' && style.flexDirection !== 'column' ? 'row' : 'column'
        }`;
      } else {
        elSelector = `StackLayout `;
      }
    }

    str += `<${elSelector} `;

    // TODO: spread support for nativescript angular
    ...
    for (const key in json.bindings) {
      ...
      if (key.startsWith('on')) {
        ...
        // str += ` [attr.${key}]="${code}" `; // not sure if attr. syntax is needed for {N}
        str += ` [${key}]="${code}" `;
      } else {
        str += `[${key}]="${code}" `;
      }
...
const processNativescriptAngularCode =
  ({
    contextVars,
    outputVars,
    domRefs,
...
const DEFAULT_OPTIONS: ToNativescriptAngularOptions = {
  preserveImports: false,
  preserveFileExtensions: false,
};

export const componentToNativescriptAngular: TranspilerGenerator<ToNativescriptAngularOptions> =
  (userOptions = {}) =>
  ({ component: _component }) => {
    ...
    options.plugins = [
      ...
      CODE_PROCESSOR_PLUGIN((codeType) => {
        switch (codeType) {
          case 'hooks':
            return flow(
              processNativescriptAngularCode({
                replaceWith: 'this',
                contextVars,
                outputVars,
          ...
          case 'bindings':
            return (code) => {
              const newLocal = processNativescriptAngularCode({
                contextVars: [],
                outputVars,
    ...
    let template = json.children
      .map((item) => blockToNativescriptAngular(item, options, { childComponents }))
      .join('\n');
    if (options.prettier !== false) {
    ...
    const dataString = getStateObjectStringFromComponent(json, {
      format: 'class',
      valueMapper: processNativescriptAngularCode({
        replaceWith: 'this',
        contextVars,
        outputVars,
    ...
    Object.entries(json.meta.nativescriptAngularConfig || {}).forEach(([key, value]) => {
  ...
  `;
    ...
    if (options.standalone !== true) {
      str = generateNativescriptNgModule(str, json.name, componentsUsed, json, options.bootstrapMapper);
    }
    if (options.plugins) {
